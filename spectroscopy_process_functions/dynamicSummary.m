function [amp_all, nmrFit, nmrFit_ppm, dyn, detrend_sine, fitted_sine, detrend_peaks, fitted_peaks] = ...
    dynamicSummary(raw_path, dyn_path, BHs, save_fig_flag, rbc_axis_lim, varargin)
...
%   detrend_sine/peaks: structure containing detrended oscillations
%   fitted_sine/peaks: structure containing fits to detrended oscillations
...
%% This section of the code is not needed as we want to see both "sine" and
% "peaks" fitting
...
[amp_sine, detrend_sine, fitted_sine] = calculateOscillationAmps(dyn, BHs, b,'sine','oscType','sine');
if ~isfield(dyn,'snrs')
    dyn.snrs = zeros(1,length(dyn.t));
end 
amp_sine.snr = mean(dyn.snrs(BHstart:BHend));
...
%%%%%%%%%%%%%%%%%%%%%% Sine Fit figures %%%%%%%%%%%%%%%%%%%%%%%%
% First, plotting dynamics, oscillations, dynamic SNR, static spectroscopy 
% for the sine fit, sine is only for oscillations but keeping all the
% figures for sine fit to prepare the slides seamlessly
...
plotOscillations(dyn, BHs, detrend_sine, fitted_sine, rbc_axis_lim, save_fig_flag, oscSave_path)
...
close all
%%%%%%%%%%%%%%%%%%%%%% Peaks Fit figures %%%%%%%%%%%%%%%%%%%%%%%%
% Second, plotting dynamics, oscillations, dynamic SNR, static spectroscopy 
