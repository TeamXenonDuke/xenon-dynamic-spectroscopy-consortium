...
classdef NMR_Fit_v < NMR_Mix_v
    ...
    properties
        ...
        ci_fwhmG; % 95% confidence interval of FWHM
    ...
    methods
        function obj = applyGlobalPhaseShift(obj, phaseShiftInDegrees)
            ...
            obj = applyGlobalPhaseShift@NMR_Mix_v(obj,phaseShiftInDegrees);
        ...
        function obj = NMR_Fit_v(time_domain_signal, t, ...
            ...
            obj = obj@NMR_Mix_v(area,freq,fwhm,fwhmG,phase);
        ...
        function obj = resetComponents(obj,area, frequency, fwhm, fwhmG, phase, varargin)
            ...
            if(nargin > 5)
                ...
                obj.ci_fwhmG = varargin{4};
                obj.ci_phase = varargin{5};
            end
        ...
        function obj = sortByFreq(obj)
            ...
            if(~isempty(obj.ci_area))
                ...
                obj.ci_fwhmG = obj.ci_fwhmG(sortIdx,:);
        ...
        function obj = setBounds(obj, area_lb, area_ub, freq_lb, freq_ub,...
                fwhm_lb, fwhm_ub, fwhmG_lb, fwhmG_ub, phase_lb, phase_ub)
            ...
            % signal..
            obj.ub = [area_ub(:)'; freq_ub(:)'; fwhm_ub(:)'; fwhmG_ub(:)'; phase_ub(:)'];
            obj.lb = [area_lb(:)'; freq_lb(:)'; fwhm_lb(:)'; fwhmG_lb(:)'; phase_lb(:)'];
            ...
            if(~isempty(obj.area))
                ...
                if(any(obj.fwhmG < obj.lb(4,:)) | any(obj.fwhmG > obj.ub(4,:)))
                    lbbad = obj.fwhmG < obj.lb(4,:)
                    obj.fwhmG(lbbad) = obj.lb(4,lbbad);
                    
                    ubbad = obj.fwhmG > obj.ub(4,:)
                    obj.fwhmG(ubbad) = obj.ub(4,ubbad);
                    warning('Bad fwhmG bound');
                end
                if(any(obj.phase < obj.lb(5,:)) | any(obj.phase > obj.ub(5,:)))
                    lbbad = obj.phase < obj.lb(5,:)
                    obj.phase(lbbad) = obj.lb(5,lbbad);
                    ubbad = obj.phase > obj.ub(5,:)
                    obj.phase(ubbad) = obj.ub(5,ubbad);
...
%             else
                ...
                for iComp = 1:length(obj.area)
                    disp([sprintf('%8.3e',obj.area(iComp)) ' ' ...
                        ...
                        sprintf('%8.2f',abs(obj.fwhm(iComp))-obj.lineBroadening) '  ' ...
                        sprintf('%8.2f',abs(obj.fwhmG(iComp))-obj.lineBroadening) '  ' ...
                        sprintf('%+9.2f',obj.phase(iComp))]);
                end
%             end
        end
    
        function describe_ppm(obj,xeFreqMHz)
            % This function will report on the fitted component
            % intensities, frequencies, linewidths, and phases in ppm using
            % the last component as a reference. Note that if line
            % broadening is used, this method subtracts off the line
            % broadening from the fitted values, thus reporting them as if
            % there was no line broadening applied
            
            nComp = length(obj.area);
            
            disp('area (arbs)  Freq (Hz)  Linewidth(Hz)  Phase(degrees)');
            for iComp = 1:nComp
                disp([sprintf('%8.2f',obj.area(iComp)/obj.area(2)) ' ' ...
                    sprintf('%+8.2f',(obj.freq(iComp)-obj.freq(nComp))/xeFreqMHz)  '  ' ...
                    sprintf('%8.2f',(obj.fwhm(iComp)-obj.lineBroadening)/xeFreqMHz) '  ' ...
                    sprintf('%8.2f',(obj.fwhmG(iComp)-obj.lineBroadening)/xeFreqMHz) '  ' ...
                    sprintf('%+9.2f',obj.phase(iComp))]);
            end
        end
    end
